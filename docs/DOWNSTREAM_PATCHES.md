...
| 3 | `release-quality-gate` | `dcc944fd`, `9cf6562f` | 发布前运行完整 Go 测试，隔离 default/classic 前端依赖，三个平台构建完成后统一创建 GitHub prerelease | fork infrastructure | `go test ./...`、default/classic build、Linux/macOS/Windows build | permanent |
...
git cherry-pick -x dcc944fd 9cf6562f
```
