...
2. 部署 `ghcr.io/streamsc/new-api-pro:v1.0.0-rc.25-pro.3`，确认启动日志没有数据库、Redis 或渠道设置错误。
3. 验证渠道页显示“在途 / 上限”，并确认标准 Relay 达到渠道上限时直接返回 429。
4. 验证 `/api/status`、登录、普通 Chat/Responses、音频 SSE、渠道测试、充值/兑换和 Sentinel 连接。
5. 抽查使用日志中的 cached token、reasoning effort、条件倍率和最终扣费。

回滚本次渠道并发功能时停止 `.3` 实例并重新部署 `v1.0.0-rc.25-pro.2`。由于没有数据库结构迁移，正常回滚不需要恢复备份；仅在确认数据已损坏时恢复升级前备份。
